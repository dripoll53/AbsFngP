...
class Configuration(object):
    ...
    epochIn = 2 # DRR default EPOCH  for history = self.cae.fit JUN26/20
    FitBatchSize=200 # DRR default batch size for model fitting (self.cae.fit) JUN26/20
    fracVal = .10
    ...
    # DOGS and catsdogs dataset parameters
    #specify the dimensionality of the last layer
    dogs_rep_dim = 128
    #specify normal class in DOGS by default digit 0 is considered as normal
    dogs_normal = 0 # select the y_train(y_test) value ClsA = 0 has initial y=1; ClsB = 1 has initial y=0

    NUMoutliers = 300    # number of outliers to use
    # ADI dataset parameters
    #specify the dimensionality of the last layer
    adi_rep_dim = 128
    #specify normal class in ADI by default digit 0 is considered as normal
    adi_normal = 0 # SITE1= 0, SITE2=1
    ADI_DATA_IN = "/home/../ADI_DB/dataA1/"
    REPORT_OUTDIR = "/home/reports/figures/" + "CX" + "ADI/RCAE/"
    SAVE_MODEL_DIR =  REPORT_OUTDIR + "models/"
    # DRR Added
    icnt = 0
    MYDATA="NONE"    #   CIFAR, DOGS
    METHOD="NONE"    #   RCAE, OCNN, DSDVV, DCAE ?
